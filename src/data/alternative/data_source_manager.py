...
class EconomicIndicatorsSource(BaseDataSource):
    def __init__(self, config: DataSourceConfig):
        super().__init__(config)
        self.indicators = []
    ...
    async def _fetch_economic_indicator(self, indicator: str, start_date: datetime, end_date: datetime) -> Dict[datetime, float]:
        try:
            ...
            indicator_configs = {
                ...
                'INFLATION': {'base': 2.0, 'volatility': 0.3},
            ...
            try:
                if isinstance(point.value, (int, float)):
                    if point.value >= 0:  # Economic indicators should be non-negative
                        ...
                        if indicator == 'INFLATION' and point.value > 10:
                            continue  # Inflation rarely exceeds 10%
...
class AlternativeDataManager:
    ...
    def __init__(self, db_adapter, cache):
        self.db = db_adapter
        self.cache = cache
    ...
    async def fetch_alternative_data(
        ...
        """
        try:
            logger.info(f"Fetching alternative data for {len(symbols)} symbols, {len(data_types)} data types")
    ...
    def _serialize_data_for_cache(self, data: Dict[str, List[DataPoint]]) -> Dict:
        ...
        for data_type, data_points in data.items():
            ...
            for point in data_points:
                ...
                serialized[data_type].append(serialized_point)
        return serialized

    def _deserialize_cached_data(self, cached_data: Dict) -> Dict[str, List[DataPoint]]:
        deserialized = {}
        for data_type_str, data_points in cached_data.items():
            deserialized[data_type_str] = []
            for point_data in data_points:
                ...
                deserialized[data_type_str].append(data_point)
        return deserialized
        ...
        try:
            ...
            for source_id, source in self.data_sources.items():
                if source.config.enabled:
                    ...
                    # Calculate quality score with error penalty
    ...
    async def get_available_data_types(self) -> List[str]:
        return [
            ...
            DataSourceType.EARNINGS_WHISPERS
        ]
    ...
    async def test_data_source_connectivity(self, source_id: str) -> Dict:
        try:
            ...
            if test_data:
                ...
                return {
                    'status': 'warning',
                    'message': 'No data returned, but no errors occurred'
                }
        except Exception as e:
