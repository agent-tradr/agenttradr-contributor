...
This properly integrates with the existing data infrastructure:
    ...
    - Alpha Vantage for fundamental data and news sentiment
    - FRED for economic indicators
    - Redis for caching all data flows
"""
...
class UnifiedDataManager:
    ...
    def __init__(self):
        """Initialize with configured data providers."""
        ...
        self.cache_ttl = {
            ...
            'bars_1m': 60,  # 1 minute for 1-minute bars
            'bars_5m': 300,  # 5 minutes for 5-minute bars
            'bars_daily': 3600,  # 1 hour for daily bars
    ...
    async def get_market_data(
        ...
        **kwargs
        ...
        Args:
            symbol: Stock symbol
            data_type: Type of data ('quote', 'depth', 'trades')
        ...
        """
        # Build cache key
        cache_key = f"{self.cache_config.MARKET_DATA_PREFIX}{data_type}:{symbol}"
        ...
        try:
            if data_type == 'quote':
                data = await self.ib_provider.get_quote(symbol)
            elif data_type == 'depth':
                data = await self.ib_provider.get_market_depth(symbol, **kwargs)
            elif data_type == 'trades':
                data = await self.ib_provider.get_trades(symbol, **kwargs)
            else:
                logger.warning(f"Unknown data type: {data_type}")
                return None
            
            # Cache the data
            if data and self.cache_connected:
    ...
    async def get_historical_data(
        ...
        """
        # Build cache key
        cache_key = f"{self.cache_config.MARKET_DATA_PREFIX}historical:{symbol}:{duration}:{bar_size}"
    ...
    async def get_news_sentiment(
        ...
        if self.cache_connected:
            cached_data = await self.cache.get(cache_key)
            if cached_data:
                logger.debug("Cache hit for news sentiment")
                return cached_data
    ...
    async def get_economic_indicators(
        ...
        if not indicators:
            ...
            indicators = ['VIXCLS', 'DFF', 'DGS10', 'UNRATE']
        ...
        if self.cache_connected:
            cached_data = await self.cache.get(cache_key)
            if cached_data:
                logger.debug("Cache hit for economic indicators")
                return cached_data
    ...
    async def get_options_chain(
        ...
        """
        # Build cache key
        cache_key = f"{self.cache_config.MARKET_DATA_PREFIX}options:{symbol}"
        if expiry:
            cache_key += f":{expiry}"
            ...
            if options_data:
                ...
                if self.cache_connected:
                    await self.cache.set(
                        cache_key,
                        processed,
                        ttl=self.cache_config.TTL_MEDIUM
                    )
    ...
    async def _handle_streaming_data(
        ...
        if self.cache_connected:
            cache_key = f"{self.cache_config.MARKET_DATA_PREFIX}{symbol}:quote"
            await self.cache.set(cache_key, data, ttl=self.cache_config.TTL_SHORT)
    ...
    def _get_ttl_for_data_type(self, data_type: str, bar_size: Optional[str] = None) -> int:
        ...
        if data_type == 'quote':
            ...
            return self.cache_config.TTL_MEDIUM
    ...
    def _process_news_for_trading(self, news_data: Any) -> Dict[str, Any]:
        ...
        if processed['overall_sentiment'] > 0.3 and processed['sentiment_strength'] > 0.6:
            ...
            processed['trading_signal'] = 'neutral'
    ...
    def _process_options_data(self, options_data: Any) -> Dict[str, Any]:
        ...
        # In production, would calculate greeks and other metrics
        return options_data
    ...
    def _calculate_liquidity_score(self, depth_data: Dict) -> float:
        """Calculate liquidity score from market depth."""
        if not depth_data:
            return 0.0
...
# Example usage
async def main():
    """Example usage of UnifiedDataManager."""
