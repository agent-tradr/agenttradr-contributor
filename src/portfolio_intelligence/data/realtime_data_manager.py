...
from market_data.ib_data_provider import IBDataProvider
from models.portfolio_intelligence import PortfolioPosition, StockUniverse
...
class DataSourcePriority(Enum):
    ...
    REAL_TIME = 1    # Live real-time data (preferred)
...
class RealtimeDataManager:
    ...
    async def get_current_price(self, symbol: str) -> Optional[MarketDataPoint]:
        """
        Returns the most recent price data available,
    ...
    async def get_portfolio_snapshot(self, user_id: int) -> Dict[str, Any]:
        """Returns Dict containing portfolio value, positions, and performance metrics"""
        try:
            db = next(get_db())
    ...
    async def _load_active_symbols(self) -> None:
        """Load symbols to monitor from database"""
        try:
            db = next(get_db())
    ...
    async def _start_symbol_stream(self, symbol: str) -> None:
        """Start data stream for a symbol"""
        try:
...
def get_realtime_data_manager() -> RealtimeDataManager:
    """Get or create the global RealtimeDataManager instance"""
...
async def initialize_realtime_data_manager() -> RealtimeDataManager:
    """Initialize and return the global RealtimeDataManager"""
